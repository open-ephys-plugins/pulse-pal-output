...
void PulsePal::initialize()
{
    ...
    if (foundDevice)
    { // AB 29/9/17: check if PulsePal 1st or 2nd generation is connected
...
void PulsePal::sendCustomPulseTrain (uint8_t ID, uint16_t nPulses, float customPulseTimes[], float customVoltages[])
{
    ...
    messageBytes[byteIndex] = 213;
    byteIndex++;
    if (ID == 1)
    {
        messageBytes[byteIndex] = 75;
        byteIndex++; // Op code to program custom train 1
    }
    else
    {
        messageBytes[byteIndex] = 76;
        byteIndex++; // Op code to program custom train 2
    }
    if (firmwareVersion < 20)
    {
        messageBytes[byteIndex] = 0;
        byteIndex++; // USB packet correction byte
    }
    messageBytes[byteIndex] = (uint8_t) (nPulses);
    byteIndex++;
    messageBytes[byteIndex] = (uint8_t) (nPulses >> 8);
    byteIndex++;
    messageBytes[byteIndex] = (uint8_t) (nPulses >> 16);
    byteIndex++;
    messageBytes[byteIndex] = (uint8_t) (nPulses >> 24);
    byteIndex++;
    // Times
    ...
    for (int i = 0; i < nPulses; i++)
    {
        ...
        messageBytes[byteIndex] = (uint8_t) (pulseTimeMicroseconds);
        byteIndex++;
        messageBytes[byteIndex] = (uint8_t) (pulseTimeMicroseconds >> 8);
        byteIndex++;
        messageBytes[byteIndex] = (uint8_t) (pulseTimeMicroseconds >> 16);
        byteIndex++;
        messageBytes[byteIndex] = (uint8_t) (pulseTimeMicroseconds >> 24);
        byteIndex++;
    }
    // Voltages
    float thisVoltage = 0;
    for (int i = 0; i < nPulses; i++)
    {
        ...
        if (firmwareVersion < 20)
        {
            messageBytes[byteIndex] = voltageToByte (thisVoltage);
            byteIndex++;
        }
        else
        {
            thisVoltageInt = voltageToInt16 (thisVoltage);
            messageBytes[byteIndex] = (uint8_t) (thisVoltageInt);
            byteIndex++;
            messageBytes[byteIndex] = (uint8_t) (thisVoltageInt >> 8);
            byteIndex++;
        }
    }
...
void PulsePal::syncAllParams()
{
    ...
    { // Pulse Pal 2
        ...
        for (int i = 1; i < 5; i++)
        {
            thisVoltage = PulsePal::currentOutputParams[i].phase1Voltage;
            thisVoltageInt = voltageToInt16 (thisVoltage);
            messageBytes[pos] = (uint8_t) (thisVoltageInt);
            pos++;
            messageBytes[pos] = (uint8_t) (thisVoltageInt >> 8);
            pos++;
            thisVoltage = PulsePal::currentOutputParams[i].phase2Voltage;
            thisVoltageInt = voltageToInt16 (thisVoltage);
            messageBytes[pos] = (uint8_t) (thisVoltageInt);
