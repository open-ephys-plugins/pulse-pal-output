...
PulsePalOutput::PulsePalOutput()
    : GenericProcessor ("Pulse Pal"), channelToChange (0), pulsePalVersion (0)
{
    // Init Pulse Pal
    pulsePal.initialize();
...
bool PulsePalOutput::checkParameterConsistency (int chan)
{
    ...
    if (! m_isBiphasic[chan])
        if (m_burstDuration[chan] != 0)
            consistent = (m_phase1Duration[chan] + m_interPulseInterval[chan] < m_burstDuration[chan]) && (m_burstDuration[chan] + m_interBurstInterval[chan] < m_trainDuration[chan]);
        else
            consistent = (m_phase1Duration[chan] <= m_trainDuration[chan]);
    else if (m_burstDuration[chan] != 0)
        consistent = (m_phase1Duration[chan] + m_phase2Duration[chan] + m_interPhaseInterval[chan]
                          + m_interPulseInterval[chan]
                      < m_burstDuration[chan])
